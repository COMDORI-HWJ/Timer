    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if(indexPath.section == 1 && indexPath.row == 2){
            if MFMailComposeViewController.canSendMail() {
                ...
                let sendMailErrorAlert = UIAlertController(title: "메일 보내기 오류", message: "현재 메일 앱이 설치되어 있지 않거나 이메일앱 설정이 되어 있지 않습니다. App Store에서 Apple Mail앱 다운로드 또는 이메일 설정을 확인해 주시기를 바랍니다.", preferredStyle: .alert)
